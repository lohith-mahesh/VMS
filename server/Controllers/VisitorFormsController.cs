...
public sealed class VisitorFormsController(IVisitorRequestService service) : ControllerBase
{
    ...
    public async Task<IActionResult> Submit(Guid id, SubmitVisitorFormDto input, CancellationToken cancellationToken)
    {
        try { return Ok(await service.SubmitVisitorFormAsync(id, input, cancellationToken)); }
    ...
    public async Task<IActionResult> AdditionalResponse(Guid id, SubmitVisitorFormDto input, CancellationToken cancellationToken)
    {
        try { return Ok(await service.SubmitAdditionalVisitorFormAsync(id, input, cancellationToken)); }
        catch (KeyNotFoundException exception) { return NotFound(new { error = exception.Message }); }
        catch (ArgumentException exception) { return BadRequest(new { error = exception.Message }); }
    }
}
