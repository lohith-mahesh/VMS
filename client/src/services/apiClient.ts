...
apiClient.interceptors.response.use((response) => response, (error) => { logger.error('API request failed', { method: error.config?.method, baseUrl: error.config?.baseURL, url: error.config?.url, status: error.response?.status, backend: error.response?.data, message: error.message }); return Promise.reject(error) })
...
export type VisitorRequestDetail = { id: string; requestNumber: string; visitor: { fullName: string; companyName: string; citizenship: string; nationality: string; country: string; designation: string; email: string; phone: string; idType: string; idLast4: string; visitorType: string }; purpose: string; visitingCompany: string; visitingSite: string; visitPurposeType: string; currentStatus: string; visitorFormId?: string; visitorFormIds?: string[]; visitorForms?: Array<{ id: string; status: string; fullName: string }>; formVersions?: Array<{ id: string; version: number; fullName: string; citizenship: string; nationality: string; country: string; company: string; designation: string; idType: string; idLast4: string; assets: string; createdAt: string }>; visitDays: Array<{ id: string; visitDate: string; expectedArrivalTime?: string; expectedDepartureTime?: string; status: string }>; assets: Array<{ id: string; assetType: string; description: string; serialNumber: string; verificationStatus: string }>; auditHistory: Array<{ id: string; action: string; details: string; createdAt: string }> }
...
export async function submitVisitorForm(id: string, input: SubmitVisitorForm) { return (await apiClient.post<VisitorRequestDetail>(`/api/visitor-forms/${id}/submit`, input)).data }
export async function submitAdditionalVisitorForm(id: string, input: SubmitVisitorForm) { return (await apiClient.post<VisitorRequestDetail>(`/api/visitor-forms/${id}/additional-response`, input)).data }
export type VisitorForm = { id: string; visitorRequestId: string; requestNumber: string; status: string; fullName: string; citizenship: string; nationality: string; country: string; designation: string; companyName: string; officeCity: string; officeCountry: string; telephone: string; email: string; idType: string; idLast4: string; assets: Array<{ assetType: string; description: string; serialNumber: string }> }
export type SubmitVisitorForm = Omit<VisitorForm, 'id' | 'visitorRequestId' | 'requestNumber' | 'status'> & { passportNumber: string; visaNumber: string; governmentIdNumber: string }
